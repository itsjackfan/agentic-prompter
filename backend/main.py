...
user_input = """
I'd you to generate a daily schedule based on the following CSV data:
Travel to volley	20 mins	12:40-1:00
Travel from volley	20 mins	2:40-3:00
Volleyball	90 mins	Yes 1:00-2:30
AOPS With Dad	60 mins	No
Mathcounts	90 mins	No
Biology	60 mins	No
ESS	90 mins	No
Piano	60 mins	No
Duolingo	30 mins	Yes 9:30-10:00
SAT Book	30 mins	No
Coding HW	60 mins	No
Dinner 	30 mins	Yes 6:30-7:00
Lunch	30 mins	Yes 12:30-1:00

Specifically, the three columns outline the task name, the duration, and whether or not the task has a fixed time. The schedule should take place from 9 AM to 10 PM each day. Please take care to obey durations and consider the difficulty and enjoyment of particular tasks.
"""
...
def decomp_prompt(prompt):
    decomp_agent = Agent('ollama:phi4',
                         ...
                         2. Breakdown into Subtasks: Decompose the task into minimal actionable steps. The first step should always be to evaluate the question and determine what context is still missing. The next step should be to either get context from the user, specifying what context is missing, through the question-answerer, or to address these requirements through a web search if they are general knowledge not specific to the projectpy. Each ensuing step should then be accomplishable by a single LLM prompt, a single snippet of code, one suggestion, or something at the same level of simplicty.
